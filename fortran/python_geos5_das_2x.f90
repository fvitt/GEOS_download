...
! loop over years,months, days
  ...
  outFileName=trim(path_out)//'/Y'//trim(year)//'/M'//trim(month)//'/D'//trim(day)// &
              '/GEOS5_19x2_'//trim(year)//trim(month)//trim(day)//'.nc'

  print*,' inFileName = '//trim(inFileName)
  print*,' outFileName = '//trim(outFileName)

  call regrid_met_data( newLats, newLons, inFileName, outFileName, rc )
